  ...
  const load = async () => {
    ...
    if (!error && data) {
      ...
      setCta(
        (v.cta && v.cta.length
          ? v.cta
          : ["Folge für mehr.", "Speichere & probier’s heute aus.", "Frag in den Kommentaren."]
        ).join("\n")
      );
      ...
      <div className="p-4 rounded-2xl border bg-white dark:bg-neutral-800 space-y-3">
        ...
        <div className="grid md:grid-cols-3 gap-3">
          ...
          <div>
            ...
            <textarea
              className="w-full h-28 px-3 py-2 rounded-lg border bg-white dark:bg-neutral-700"
              value={cta}
        ...
        <div>
          ...
          <textarea
            className="w-full h-24 px-3 py-2 rounded-lg border bg-white dark:bg-neutral-700"
            value={hashtags}
