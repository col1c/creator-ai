...
export default function ReferralCard() {
  ...
  const claim = async () => {
    ...
    try {
      ...
      if (!token || !claimCode.trim()) throw new Error("Bitte Code eingeben.");
      const r = await fetch(`${API_BASE}/beta/invite/use`, {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ code: claimCode.trim() })
      });
      if (!r.ok) throw new Error("Ungültiger Code");
      alert("Referral/Invite eingelöst 🎉");
      setClaimCode("");
    } catch (e: any) {
      alert(e?.message || "Einlösen fehlgeschlagen.");
    } finally {
      setBusy(false);
    }
  };
    ...
    <div className="border rounded-2xl p-4">
      ...
      <div className="mt-4">
        ...
        <div className="flex gap-2">
          ...
          />
          <button className="border rounded-xl px-3" onClick={claim} disabled={busy}>Einlösen</button>
        </div>
      </div>
    </div>
  );
